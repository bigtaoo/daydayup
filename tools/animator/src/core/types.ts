...
export interface BoneDef {
  ...
  outerW?: number;   // tubular shaft draw width (outer), when this bone has a visible shaft
  innerW?: number;   // tubular shaft draw width (inner)
  bodyR?: number;    // draw/hit radius for a circular module at the bone's tip (e.g. shell, eye, a weapon socket)
  label: string;
}
...
export interface AttachmentPoint {
  id:         string;   // 'shadow' today; more ids can be added per-rig later
  label:      string;
  parentBone: string;
  offsetX:    number;
  offsetY:    number;
