...
export class InteractionController {
  ...
  private readonly drawOrderReversed: readonly string[];
  ...
  constructor(
    ...
    this.drawOrderReversed = [...rig.drawOrder].reverse();
  ...
  private onMouseDown(e: MouseEvent): void {
    ...
    const worldPose = this.rig.computeFK(this.state.rootX, this.state.rootY, wp, this.state.boneLengthScales);
  ...
  private onMouseMove(e: MouseEvent): void {
    ...
    const { x, y } = this.renderer.toStageCoords(e.clientX, e.clientY);
    const frame = this.animCtrl.getCurrentFrame();
    const worldPose = this.rig.computeFK(this.state.rootX, this.state.rootY, frame, this.state.boneLengthScales);
  ...
  private findBoneAt(
    x: number,
    y: number,
    worldPose: ReturnType<Rig['computeFK']>,
  ): string | null {
    // Circular modules first (shell/eye/belly/sockets) — front-first via reversed draw order.
    for (const boneId of this.drawOrderReversed) {
      const bone = this.rig.boneMap.get(boneId);
      const pos  = worldPose.get(boneId);
      if (!bone?.bodyR || !pos) continue;
      const dx = x - pos.ex, dy = y - pos.ey;
      if (Math.sqrt(dx * dx + dy * dy) <= bone.bodyR + 4) return boneId;
    }

    // Then tubular shafts — closest segment in reversed draw order (front first)
    let best: string | null = null;
    let bestDist = Infinity;

    for (const boneId of this.drawOrderReversed) {
      const bone = this.rig.boneMap.get(boneId);
      if (!bone || bone.bodyR || !this.rig.selectableBones.includes(boneId)) continue;
      const pos = worldPose.get(boneId);
      if (!pos) continue;
