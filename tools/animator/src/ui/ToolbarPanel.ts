...
/** The orb-core's preset clip names (design/12: hover-bob/lean/squash-stretch —
 *  no walk cycle). Kept as a fixed six-slot list (same shape as the ported
 *  original) so App.ts's resetToDefaults() and the preset picker agree. */
...
export class ToolbarPanel {
  ...
  private bindExisting(): void {
    ...
    document.getElementById('btn-presets')?.addEventListener('click', () => {
      const name = prompt(`Load preset (${PRESET_NAMES.join(', ')}):`);
      if (!name) return;
      const trimmed = name.trim();
      if (!(PRESET_NAMES as readonly string[]).includes(trimmed)) { this.bus.emit('error', `Unknown preset: ${trimmed}`); return; }
