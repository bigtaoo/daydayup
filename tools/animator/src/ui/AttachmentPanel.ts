/**
 * Attachment Points panel — edits shadow (and any future attachment point) offsets.
...
export class AttachmentPanel {
  ...
  /** Bones available as parent — every bone in the active rig, root included. */
  private readonly boneOptions: readonly string[];
  ...
  constructor(
    ...
    this.boneOptions = ['root', ...rig.selectableBones];
    this.build();
      ...
      section.innerHTML = `
        ...
        <div class="prop-row">
          ...
          <select id="apt-${pt.id}-bone" style="flex:1;font-size:11px">
            ${this.boneOptions.map(b => {
              const def = this.rig.boneMap.get(b);
        ...
        <div class="prop-row" style="margin-top:4px">
          ...
          <input class="prop-input" type="number"
            ...
            step="1" style="width:55px" title="Ellipse half-width; leave empty for the rig default">
        </div>
        <div class="prop-row">
          ...
          <input class="prop-input" type="number"
            ...
            step="1" style="width:55px" title="Ellipse half-height; leave empty for the rig default">
        </div>
        ` : ''}
      `;
