...
- [x] Art pipeline (Phase 5): `.tao` rig runtime, full 3-character + boss/critter/brute/floater roster art, a distinct sprite for every ranged/melee weapon id, per-element biome floor/wall art, hub background + button icons + the Forger NPC, UI widget kit, post-processing/particles, all four fidelity-roadmap custom shaders — energy shield, hit-flash outline, dissolve-on-death, heat-haze (`design/01` 5.4) — every asset is AI-generated placeholder art awaiting an authored replacement (`design/12` 5.3), normal-map lighting is blocked on that same real-art pass, and WeChat device verification is still outstanding
