...
export class PortalPrompt {
  ...
  /** `show` is the caller's already-computed "at a non-last-floor checkpoint AND
   *  standing near the portal" condition — kept out of this class so Game doesn't have
   *  to duplicate it between here and RoomBuilder.setPortalOpen (which needs the same
   *  checkpoint half without the proximity half). */
  update(s: GameState, show: boolean): void {
    this._isOpen = show;
