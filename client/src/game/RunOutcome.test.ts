...
interface RecordedHost extends RunOutcomeHost {
  ...
  readonly shown: { title: string; lines: readonly string[] } | undefined;
}
...
function mockHost(localOwner = 0): RecordedHost {
  ...
  let hudHidden = false;
  let shown: { title: string; lines: readonly string[] } | undefined;
  return {
    localOwner,
    ...
    showOutcomeScreen: (title, lines) => { shown = { title, lines }; },
