...
/**
 * The TIER a door fixture stands at: that of the wall it is cut into (`doorRender.ts`).
 *
 * A tier rather than a height, so the door can then be handed to `wallJoins` as an ordinary
 * `WallRun` and learn which of its own edges are buried in the wall it interrupts — a doorway's
 * cap has to run into the flanking runs' caps without either side drawing an "I end here" coping
 * across the join.
 *
 * A door's `passageAabb` is a hole in a wall, so the runs that ABUT it — sharing an edge along the
 * gap and overlapping it on the other axis — are that wall, arriving here as one run per side (or
 * as several, since a boundary between two rooms is authored by both of them and a passage rect
 * spans both rows). The door takes the SHORTEST of them, and the direction of that choice is the
 * point:
 *
 * - A door in a room boundary between two vertically stacked rooms is flanked by KERBS
 *   (`wallGeometry.WALL_H_KERB`, 22 px), which are low precisely because a room's floor lies
 *   immediately north of them and anything tall there stands between the camera and the player.
 *   A door is no more entitled to that space than the wall it interrupts is, so it must not
 *   inherit a neighbour's full perimeter height just because one is also in reach. Its
 *   legibility comes from the hazard bloom instead (`doorRender`'s `GLOW_*`).
 * - Taking the shortest also means a door never out-tops the mass it is set into, so its cap
 *   cannot spill north over stone that is holding its own crown course — the same rule
 *   `wallJoins`' tuck exists to protect, obtained here for free.
 *
 * `null` when nothing abuts the passage at all (a mode with no wall model, or a hand-built
 * fixture in a test); the caller decides what to do with that rather than getting a silent
 * default from geometry that does not know the room.
 */
export function doorFlankTier(door: RectPx, runs: readonly WallRun[]): WallTier | null {
  let best: WallTier | null = null;
  for (const run of runs) {
    if (!abutsAlongGap(door, run.rect)) continue;
    if (best === null || wallHeight(run.tier) < wallHeight(best)) best = run.tier;
  }
  return best;
}
