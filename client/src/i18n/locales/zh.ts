/**
 * Chinese translation (design/17-i18n.md). Must mirror `en.ts`'s exact key shape —
 * `Translations<typeof en>` (../index.ts) makes a missing/extra key a compile error.
 */
import type { Translations } from '../index';
import type { en } from './en';

export const zh: Translations<typeof en> = {
  mainMenu: {
    title: 'DAYDAYUP',
    subtitle: '深入·撤离·生存',
    play: '开始',
    squad: '组队',
    account: '登录',
    settings: '设置',
    greeting: '你好，{username}',
  },
  pauseMenu: {
    title: '已暂停',
    resume: '继续',
    settings: '设置',
    quit: '返回锻造场',
  },
  settings: {
    title: '设置',
    master: '总音量',
    sfx: '音效',
    music: '音乐',
    mute: '静音',
    unmute: '取消静音',
    autoAimOn: '自动瞄准：开',
    autoAimOff: '自动瞄准：关',
    language: '语言：{name}',
    back: '返回',
  },
  forge: {
    title: '锻造场',
    hint: '[↑↓]/[1-9]/[C]/[X]/[Enter] 键盘快捷键仍然可用',
    backButton: '← 菜单',
    clearLoadout: '清空装备',
    startRun: '开始行动 ▸',
    pagePrevButton: '‹ 上一页',
    pageNextButton: '下一页 ›',
    charStats: '{skin}（{hp}生命 / {sh}护盾）',
    noneAutoPistol: '（无 → 自动配备手枪）',
    moreAvailable: '还有 {count} 件可购买',
    materialsLine: '材料   {bank}   |   已拥有角色：{ownedChars}',
    loadoutLine: '装备     {loadout}   （{count}/{max}）',
    storeLine: '商店（演示期间免费）：{items}  — [B] 获取下一件',
    lockedFind: '未解锁（需寻获）',
    lockedSource: '未解锁（{source}）',
    craftable: '可打造',
    needMaterials: '材料不足',
    stagedTag: '  ▸已装备×{count}',
    pageLabel: '第 {current}/{total} 页',
    equippedHeader: '当前装备：{id}',
    candidateHeader: '候选：{id}',
  },
  hud: {
    swapHint: '[E] 更换',
    weaponLine: '{name} [{rarity}]（{kind}）伤害 {damage}',
    weaponNone: '武器：无',
    buffsSuffix: '  增益 {count}',
    escalationSuffix: '（+{n}/回合）',
    pvpLine: '{skin}   PvP 竞技场   区域阶段 {stage}{escalation}   存活 {alive}/{total}   分数 {score}{buffs}',
    pveLine: '{skin}   楼层 {floor}/{floorCount}   房间 {room}/{rooms}   敌人 {enemies}   已存 {banked}   分数 {score}{buffs}',
    allyLine: '队友（{skin}）   {status}',
    allyDowned: '倒地 — {seconds}秒后流血过多',
    allyHp: '生命 {hp}/{maxHp}',
    checkpointTitle: '楼层已清空 — 检查点',
    checkpointExtract: '长按 [E] 撤离：立即存入 {pending} 份材料并安全结束本轮',
    checkpointDescend: '轻按 [E] 下降至第 {floor} 层：风险更高，但可继续游玩',
  },
  auth: {
    title: '账户',
    login: '登录',
    register: '注册',
    changePassword: '修改密码',
    logout: '登出',
    back: '返回',
    usernamePlaceholder: '用户名',
    usernameRequired: '请输入用户名。',
    passwordPlaceholder: '密码',
    currentPasswordPlaceholder: '当前密码',
    newPasswordPlaceholder: '新密码',
    loginFailed: '登录失败，请重试。',
    registerFailed: '注册失败，请重试。',
    passwordChanged: '密码已修改。',
    passwordChangeFailed: '无法修改密码。',
    loggedInAs: '已登录：{username}',
    playingAsGuest: '以访客身份游玩',
  },
  party: {
    title: '组队',
    create: '创建队伍',
    join: '输入邀请码加入',
    startMatching: '开始匹配',
    leave: '离开队伍',
    back: '返回',
    partyClosed: '队伍已解散。',
    createFailed: '创建队伍失败，请重试。',
    codePlaceholder: '邀请码',
    invalidCode: '邀请码无效或队伍已满。',
    startFailed: '开始匹配失败，请重试。',
    codeLine: '邀请码：{code}',
    you: '你',
  },
  results: {
    mainMenuButton: '主菜单',
    extractedTitle: '撤离成功',
    defeatTitle: '战败',
    victoryTitle: '大吉大利',
    eliminatedTitle: '已淘汰',
    confirmHint: '按开火键返回装备界面',
    timeLine: '用时 {m}:{ss}',
    floorLine: '楼层 {floor}/{floorCount}',
    materialsBanked: '+{count} 份材料已存入',
    scoreLine: '分数 {score}',
    fellOnFloor: '倒在第 {floor}/{floorCount} 层',
    materialsLost: '本层材料已全部丢失',
    placeOf: '第一名，共 {total} 名',
    placedOfTotal: '第 {place}/{total} 名',
  },
  toast: {
    heal: '+1 生命',
    newWeapon: '新武器',
    buffNamed: '增益：{id}',
    buffGeneric: '增益',
    materialQty: '+{qty} {material}',
    materialFallback: '材料',
  },
  compareCard: {
    damage: '伤害',
    type: '类型',
    fireRate: '射速',
    spread: '散射',
    speed: '速度',
    swing: '挥砍',
    arc: '范围角度',
    reach: '距离',
    deflect: '格挡',
    yes: '是',
    no: '否',
    vs: '对比',
  },
};
