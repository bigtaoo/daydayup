...
| Minimap / room progress | `room` + cleared count (`05`/`09`) | ✅ **PvE shipped**: `FloorProgress` (`client/src/game/ui/FloorProgress.ts`, `state.floorStages.length`/`roomIndex`) — a progress TRACK (done/current/upcoming nodes, capstone marked), NOT the same widget as the PvP room-graph `Minimap` below it. PvE loads one room live at a time (ROADMAP 1.3), so there's no co-resident spatial layout to fit into a box like PvP's `ArenaMap` gives — the track is the honest shape for PvE's actual data. A plain-language one-line legend under the dots (`green=done amber=now diamond=checkpoint`) shipped 2026-08-01 — the bare colored shapes had no self-evident meaning on first sight. ✅ **PvP shipped**: a separate room-graph `Minimap` widget (ROADMAP 4.x) reading `content/arenas.ts`'s room/door graph + `state.zone`, unaffected by the PvE track above. |
| Extraction / checkpoint prompt | `wavesExhausted` + `enemies.length` (`ExtractionSystem`) | ✅ **Shipped, rewritten 2026-08-01**: a centered banner (not a corner text line) shown only at a non-last-floor checkpoint — `FLOOR CLEARED — CHECKPOINT` / `HOLD [E] to EXTRACT: bank N materials...` / `TAP [E] to DESCEND to Floor N+1...`, with the real pending-material count and next floor number filled in (see ROADMAP's 2026-08-01 entry for why the original one-line version was unreadable). |
