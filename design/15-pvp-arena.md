...
- **One ~60-room hand-authored map at launch, produced by a dedicated map editor** — not a procedural layout. The editor is the authority on room layout, doors, monster placement, loot markers, and per-cell hazard traits; the engine only consumes the produced data. ⚠️ **Half shipped.** The EDITOR is real (`tools/map-editor`, three modes, save-blocking `validate.ts`) and a 60-room map is wired in and playable: `world/arenas/arena_prototype_60.json` → `client/src/game/match/arenaCatalog.ts`'s `ARENA_CATALOG`, browser-verified two-tab building the identical 134×86 config off it. **But that map is procedurally generated, not hand-authored, and it has no geometry at all** — 60 identical 10×10 rooms on a regular lattice with `solids: []` everywhere. Since `roomGeometry` derives walls only from `solids`, the room rects and all 71 doors are LOGICAL ONLY: an actor walks straight across the map through three rooms and two gaps without touching anything (measured), so every chokepoint is fictional and the two structural rules above (explicit door adjacency; room membership by rect) describe a graph nothing physically enforces. **Its pillars and loot markers are also authored as ABSOLUTE coordinates** where this schema is room-relative (only `rectGrid` is absolute — see `buildArenaRoomRects`), so the room offset is applied to an already-absolute number: `room_0_0`'s pillar, written `(16,16)`, is built at `(24,24)`, and 90 of the 120 pillars/markers land off the 134×86 map entirely. `cellTraits` and `spawns` are authored correctly. It satisfies `validate.ts`, which answers a different question than this bullet asks. Measure it with `npm run audit:arena -w client` (`engine/content/arenaMetrics.ts`); walk it in one tab with `?arena=arena_prototype_60`. `landing_basic` remains a small synthetic fixture reserved for the plain `?arenaDemo=1` dev harness. **Authoring the real launch map is open** — see ROADMAP's "The launch arena is a placeholder that passes validation".
