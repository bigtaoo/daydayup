...
- **One ~60-room hand-authored map at launch, produced by a dedicated map editor** — not a procedural layout. The editor is the authority on room layout, doors, monster placement, loot markers, and per-cell hazard traits; the engine only consumes the produced data. ✅ **Shipped:** `world/arenas/arena_prototype_60.json` (a validated 60-room `ArenaMap`, matching the schema below exactly) is wired into `client/src/game/arenaCatalog.ts`'s `ARENA_CATALOG`, browser-verified two-tab building the identical 134×86 config off it; `landing_basic` remains a small synthetic fixture reserved for the `?arenaDemo=1` dev harness.
