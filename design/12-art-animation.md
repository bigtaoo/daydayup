...
- The **two orbiting socket bones rotate to the exact aim angle**; the mounted weapon module follows, so the muzzle (or the blade's sweep) always points at the reticle. **Since 2026-09-02 the ACTIVE module also *travels* around the core to the aim**, at its socket bone's own reach, in the ground plane — it used to sit at that bone's fixed tip and merely spin, which put the drawn gun up to a socket-length off the line its own bullets fly along and drew a visible arc out of the barrel (`01`'s bullet-spawn section has the measurement and the fix; the idle module still hangs on its own bone, pointing outward). This is what `13`'s "two weapon modules that orbit it" always described.
