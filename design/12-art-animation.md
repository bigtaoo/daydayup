...
>
> **Update (2026-09-06): the two mob melee weapons are the only entries in `WEAPON_DEFS` pointing at another weapon's texture, and the prompts to fix that are now written down.** `enemyclaw` and `enemymaul` (`ENGINE_VERSION` 59) borrow the player spear's and hammer's PNGs. That is a deliberate placeholder rather than a fallback — pointing them at `sword_default.png` would trip `muzzleParity`'s "never the kind default" rule, and a path with no PNG behind it would ship a missing texture — but it means a raider's claw currently reads on screen as **player gear**, i.e. as loot you could pick up, which is the opposite of what a mob's weapon should signal. What actually carries that signal at a ~40 px on-screen body is not the silhouette but the PALETTE, and the mob roster already has one, established by `gun_enemygun.png` and sampled off its pixels: dark blue-grey housing (`#202030`–`#404050`) with a **violet** crystal (`#402080`, `#502090`, `#7030B0`) and pale-lilac highlights, against the player's white-and-gold. Two complete, copy-paste-ready GPT Image 2 prompts now live in `art/weapon/prompts.md` alongside both halves of the workflow — including the two corrections a later batch would otherwise repeat: `compress.mjs --long-axis=160`, not the 320 the archived section says (every file went 320 → 160 on 2026-08-25 for the WeChat budget), and re-measure `rotationOffsetRad` rather than keeping the borrowed near-180° value.
