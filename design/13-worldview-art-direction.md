...
- **Pillar surfacing** — measured open item (2026-08-20, the scene pass after walls/characters/doors). The pillars carry hand-toned stone (`scene/pillarRender.ts`) but not the shipped SWATCHES the walls use: their cap is a flat gradient where a wall cap is `wall_<element>.png` plus an additive key light, and their body samples the elevation swatch at a scale that comes out as blurred blobs. On a live frame of level 1 they read as smooth plastic bins standing next to real stone — the same mismatch the pale-mauve palette pillars had before 2026-08-18, one step further along. Next in the scene queue; no new art needed, the swatches already exist.
