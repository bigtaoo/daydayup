...
5. **`InputSource` seam.** Route input through `LocalInputSource` → validate single-player still plays identically (golden-replay check: same seed + same input → identical end state).
6. **Server broadcast + `NetInputSource` + local prediction.** Only now add the online layer.
