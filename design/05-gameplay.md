...
below), so the real gap was doors — they now render as a real `Sprite` fixture per
...
- **Doors are always-present physical fixtures with exactly two visual states, locked/open** — never a bare gap in the wall. `art/environment/door_{locked,open}_raw.png` are the first accepted pair (`13`'s flat-cel direction: a hazard-saturated glowing barrier when locked, a desaturated inert frame when open).
