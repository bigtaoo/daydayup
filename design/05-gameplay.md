...
**(DECIDED, ROADMAP Phase 4.1 — full schema in `15-pvp-arena.md`.)** PvP is a **last-player-standing battle royale with a shrinking safe zone**, not a symmetric team arena. This revises the "3v3/4v4" framing below to an 8-player solo mode first, with team play as a reserved (not-yet-built) extension of the same model.

- **8-player solo, free-for-all, elimination + shrinking zone.** Launch mode is **8 seats, no teams** — every seat is its own `teamId`. Squads (shared `teamId` across seats, downed/revive within a squad) are a **reserved interface, not built**: the data model (`teamId` separate from seat `owner` from day one, `15`) supports it, but matchmaking, UI, and the revive rules below stay solo-only until squads are actually scheduled.
...
- **Downed/revive is a reserved interface, disabled at launch.** PvE's revive (`05` above — free channel, unlimited, bleedout-gated) does **not** carry over as-is: solo BR has no teammate to revive, so the mechanism is simply inert for now. When squads ship, PvP revive is planned to differ from PvE in two ways: it is **gated by a consumable bandage item** (not a free channel), and (per the same open question as before) downed players are likely **not** invulnerable in PvP the way they are in PvE — exact rules deferred until squads are actually scheduled (`15`).
...
- The PvP **landing-kit set** (how many, contents — a small opener, not a full loadout), the arena's own loot/drop table, the PvP HP/damage scale factor's exact value, and squad revive numbers (bandage cost, channel time, downed-vulnerability rule) — all content-tuning work against real play, not blocking (`15`).
