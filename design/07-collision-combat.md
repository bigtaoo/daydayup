...
- **Shield tuning & break-passive catalog** — `SHIELD_REGEN_DELAY`/`INTERVAL`, the revive channel length & restored-HP amount, and the set of shield-break passives (AoE burst radius/damage, knockback impulse) bound to characters (`02`/`09`). Whether a break-passive can itself break (recursive `shield_break`) needs a guard like lightning's no-recursion rule.
