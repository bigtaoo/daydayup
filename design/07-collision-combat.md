...
- **Enemy-enemy collision:** ⚠️ **shipped as blocking, not overlap** — the actor–actor decision above (step 4.3) has no faction gate ("same-plane pair" is any two actors), so an enemy walking into another enemy now blocks/pushes just like a player would, contradicting this bullet's own "lean overlap at launch" lean below. Flagging rather than silently picking a side: if enemy-enemy blocking reads badly or costs too much in a packed room during playtest, the fix is a small faction check in `resolveActorPairs` (skip a pair where both are `faction === 'enemy'`) — not implemented pre-emptively since the locked decision above doesn't ask for one and this is exactly the kind of call that needs real play, not a guess.
