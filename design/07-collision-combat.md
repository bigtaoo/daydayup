...
- **Enemy-enemy collision:** ✅ **resolved — enemies lean overlap.** `resolveActorPairs` now skips a pair where both actors are `faction === 'enemy'` (a small check ahead of the push-apart math); player-vs-player and player-vs-enemy pairs are unaffected, still pushed apart unconditionally. `ENGINE_VERSION` bumped for the outcome change.
