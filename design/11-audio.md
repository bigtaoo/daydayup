...
> **Status (2026-07-26): the event→sound seam + a synthesised backend now ship on BOTH web and WeChat; real assets + music do not.** The `AudioBus` platform seam (`platform/types.ts`), the shared procedural voice table (`platform/audioSynth.ts` — every cue is generated live, no asset files, no licensing), and the render-side event→cue map with per-frame coalescing (`Game.consumeEvents`) are built and wired on both entries. **WeChat is no longer a no-op stub**: `platform/wechat/WeChatAudio.ts` drives the identical voice table through `wx.createWebAudioContext()` (feature-detected; falls back to a true no-op if the base library lacks it — UNVERIFIED on a real lowest-base-library device, design/04 checklist item 2 still open) — closing the "the event→sound pipeline is unproven on WeChat" gap without needing any asset. Real authored SFX/music/ambience still do not exist (needs sourcing + a licence check, below) — that half of this doc remains the owner's to do. No music/ambience yet either way.
